...
System menu: **Doom menu** (Esc), **Automap** (Tab), **Always run** and **Music** toggles.
...
- Sound effects are mixed in software (`src/i_playdate_sound.c`); music is converted MUS->MIDI and played on simple Playdate synths (percussion dropped; toggle in the system menu). Config (`.cfg`) is not loaded/saved; savegames go to the game's Data folder.
